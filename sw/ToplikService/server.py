...
def api_get_guest_status():
    ...
    try:
        ...
        current_temp, setpoint = 22.0, 22.0; is_thermostat_on = False
        ...
        if r_temp and r_temp.ok:
            ...
            logging.warning(f"Greška pri dohvaćanju GET_ROOM_TEMP za {soba_id}")
        ...
        if r_status and r_status.ok:
            ...
            logging.warning(f"Greška pri dohvaćanju TH_STATUS za {soba_id}")

        logging.info(f"GOST STATUS ({soba_id}): Vraćam Temp={current_temp}, Setpoint={setpoint}, On={is_thermostat_on}")
        return jsonify({
            'success': True, 'currentTemp': current_temp,
            'setpoint': setpoint, 'isThermostatOn': is_thermostat_on
        })

    except jwt.ExpiredSignatureError:
        return jsonify({'success': False, 'message': 'Sesija istekla'}), 401
    except Exception as e:
        logging.error(f"Greška u /api/status: {e}")
        return jsonify({'success': False, 'message': 'Greška servera'}), 500
